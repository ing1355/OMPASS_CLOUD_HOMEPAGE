...
function android(props) {
  ...
  return <DocumentLayout title="안드로이드">
    <div className={styles["qrcode-mobile"]}>
      <br />
      <img
        className={styles["qrcode-mobile"]}
        width="15%"
      ...
      >
        {t(
          "안드로이드 전용 Play 스토어에서 원모어패스(OMPASS)를 다운로드하여 설치해야 합니다."
        )}
        <br />
    ...
    <DocumentTextBox>
      ...
      <DocumentBox>
        <DocumentLabel icon="❶-⑴" labelClassName={styles["number"]} title="해당 앱이 최신이 아닐 경우 업데이트 팝업창이 표시됩니다." />
        <DocumentLabel icon="→" labelClassName={styles["number"]} title="최신 업데이트를 진행하고 앱을 실행해주세요." />
        <DocumentLabel icon="❶-⑵" labelClassName={styles["number"]} title="해당 앱을 다운로드시 최초 설정이 진행됩니다." />
    ...
    <DocumentTextBox>
      ...
      <DocumentBox>
        ...
        <DocumentLabel icon="→" labelClassName={styles["number"]} title="생체 인증(지문, 얼굴인식), PIN CODE, 패턴 중 2가지 이상 등록해야 합니다." />
